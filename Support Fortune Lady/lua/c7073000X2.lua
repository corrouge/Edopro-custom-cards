--Princesse de la destiné tragique
...
function s.tgfilter(c)
	return c:IsMonster() and c:IsSetCard(SET_FORTUNE_LADY) and c:IsAbleToGrave()
end
function s.lvrescon(mustlv)
	return function(sg)
		local res,stop=sg:GetClassCount(Card.GetAttribute)==#sg,sg:GetClassCount(Card.GetAttribute)~=#sg
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local rg=Duel.GetMatchingGroup(s.tgfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil)
	local g=aux.SelectUnselectGroup(rg,e,tp,1,3,s.lvrescon(6),1,tp,HINTMSG_TOGRAVE,nil,nil,true)
	...
	e1:SetTarget(function(e,c) return not c:IsSetCard(SET_FORTUNE_LADY) and not c:IsAttribute(attributes) end)
