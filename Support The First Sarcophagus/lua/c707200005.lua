...
function s.initial_effect(c)
	...
	local e1=Ritual.CreateProc(c,RITPROC_GREATER,aux.FilterBoolFunction(Card.IsCode,CARD_URAEUS),nil,aux.Stringid(id,0),nil,nil,nil,nil,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCountLimit(1)
	e1:SetCondition(function(e,tp) return not Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_URAEUS),tp,LOCATION_MZONE,0,1,nil) end)
	c:RegisterEffect(e1)
...
s.listed_names={id,31076103,CARD_URAEUS}
